...
class PyTorchModuleWrapper:
    ...
    def predict(self, state):
        ...
        return self.nn(state_).detach().cpu().detach().numpy()
...
class DQNCombo(core.RLAgent):
    def __init__(self, params, ae, scaler, push_target_actor, push_obstacle_actor):
        ...
        with open(push_target_actor, 'rb') as file:
            ...
            self.policy.append(PyTorchModuleWrapper(actor, device=self.device))
        self.policy.append(push_obstacle_actor)

        self.combo_feature = ComboFeature(self.ae, self.scaler)

    def predict(self, state):
        state_ = self.combo_feature(state)
        ...
        if primitive == 1:
            ...
            scaler_ = self.scaler
        state_ = clutter.get_asymmetric_actor_feature_from_dict(state, self.ae, scaler_, 0, primitive=primitive)
        action = self.policy[primitive].predict(state_)
        print('PREDICT: primitive', primitive, action)
    ...
    def explore(self, state):
        ...
        state_ = clutter.get_asymmetric_actor_feature_from_dict(state, self.ae, scaler_, 0, primitive=primitive)
        action = self.policy[primitive].predict(state_)
        print('EXPLORE: primitive', primitive, action)
        return np.insert(action, 0, primitive)
    ...
    def _transitions(self, transition):
        ...
        for j in range(heightmap_rotations):
            ...
            state = self.combo_feature(transition.state, angle=angle[j])
            next_state = self.combo_feature(transition.next_state, angle=angle[j])
...
class ComboExp:
    def __init__(self, params, push_target_actor_path, push_obstacle_actor_path, vae_path, seed=0):
        self.params = copy.deepcopy(params)
        self.seed = seed
        ...
        self.autoencoder.load_state_dict(model)
        with open(normalizer_path, 'rb') as file2:
            self.normalizer = pickle.load(file2)
        ...
        self.agent = DQNCombo(params=dqn_params, ae=self.autoencoder, scaler=self.normalizer,
                            push_target_actor=push_target_actor, push_obstacle_actor=push_obstacle_actor)
