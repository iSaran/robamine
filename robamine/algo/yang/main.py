...
def main(args):
    ...
    def process_actions():
        while True:
            if nonlocal_variables['executing_action']:
                ...
                # nonlocal_variables['best_push_pix_ind']: (rotation, init_pixel)
                # nonlocal_variables['push_end_pix_yx']: end_pixel
                nonlocal_variables['best_push_pix_ind'], nonlocal_variables['push_end_pix_yx'] = \
                    utils.get_push_pix(push_predictions, trainer.model.num_rotations)
                nonlocal_variables['best_grasp_pix_ind'] = np.unravel_index(np.argmax(grasp_predictions),
                                                                            grasp_predictions.shape)

                # Visualize executed primitive, and affordances
                if save_visualizations:
                    push_pred_vis = trainer.get_push_prediction_vis(push_predictions, color_heightmap,
    ...
    while True:
        ...
        ##### Get heightmap, mask with shape (224, 224) #####
        ...
        if 'prev_color_img' in locals():
            ...
            if not is_testing:
                ...
                if sample_ind.size > 0:
                    ...
                    sample_iteration = utils.get_replay_id(trainer.predicted_value_log, trainer.label_value_log,
