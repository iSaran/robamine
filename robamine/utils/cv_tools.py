...
def extract_features(height_map, dim, plot=False):
    ...
    target_bbox = np.round( np.round(side + 0.5) / 4.0)
    target_bbox = target_bbox.astype(int)
    ...
    for i in range(8 - target_bbox[0], 8 + target_bbox[0]):
        for j in range(8 - target_bbox[1], 8 + target_bbox[1]):
            id = i * 16 + j
            object_space.append(cells[id])

            if plot:
                rgb = draw_cell(cells[id], rgb)
    ...
    features.append(target_bbox[0])
    features.append(target_bbox[1])

    return features


def find_cells_around_target(target_bbox, cells, gap = 4):
    '''
    ToDo:
    '''
    cells_around = []
